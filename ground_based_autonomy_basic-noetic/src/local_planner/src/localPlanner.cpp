...
#define stampToSec(stamp) (stamp.sec + stamp.nanosec * 1e-9)

void odometryHandler(const nav_msgs::msg::Odometry::ConstSharedPtr odom) {
  // odomTime = odom->header.stamp.seconds();
  odomTime = stampToSec(odom->header.stamp);
...
void laserToPointCloud(const sensor_msgs::msg::LaserScan::ConstSharedPtr laser,
...
void laserCloudHandler(
  ...
  laserToPointCloud(laser, laserCloud);
...
void boundaryHandler(
    const geometry_msgs::msg::PolygonStamped::ConstSharedPtr boundary) {
  boundaryCloud->clear();
...
void addedObstaclesHandler(
    const sensor_msgs::msg::PointCloud2::ConstSharedPtr addedObstacles2) {
...
void checkObstacleHandler(const std_msgs::msg::Bool::ConstSharedPtr checkObs) {
  double checkObsTime = rclcpp_clock->now().seconds();
...
int main(int argc, char **argv) {
  ...
  // Declare parameters
  nh->declare_parameter<std::string>("pathFolder",
                                     "/path/to/local_planner/paths");
  nh->declare_parameter<double>("vehicleLength", 0.6);
  nh->declare_parameter<double>("vehicleWidth", 0.6);
  ...
  nh->declare_parameter<double>("laserVoxelSize", 0.05);
  nh->declare_parameter<double>("terrainVoxelSize", 0.2);
  nh->declare_parameter<bool>("useTerrainAnalysis", false);
  nh->declare_parameter<bool>("checkObstacle", true);
  nh->declare_parameter<bool>("checkRotObstacle", false);
  nh->declare_parameter<double>("adjacentRange", 3.5);
  nh->declare_parameter<double>("obstacleHeightThre", 0.15);
  nh->declare_parameter<double>("groundHeightThre", 0.1);
  nh->declare_parameter<double>("costHeightThre", 0.1);
  nh->declare_parameter<double>("costScore", 0.02);
  nh->declare_parameter<bool>("useCost", false);
  nh->declare_parameter<int>("pointPerPathThre", 2);
  nh->declare_parameter<double>("minRelZ", -0.5);
  nh->declare_parameter<double>("maxRelZ", 0.25);
  nh->declare_parameter<double>("maxSpeed", 1.0);
  nh->declare_parameter<double>("dirWeight", 0.02);
  nh->declare_parameter<double>("dirThre", 90.0);
  nh->declare_parameter<bool>("dirToVehicle", false);
  nh->declare_parameter<double>("pathScale", 1.0);
  nh->declare_parameter<double>("minPathScale", 0.75);
  nh->declare_parameter<double>("pathScaleStep", 0.25);
  nh->declare_parameter<bool>("pathScaleBySpeed", true);
  nh->declare_parameter<double>("minPathRange", 1.0);
  nh->declare_parameter<double>("pathRangeStep", 0.5);
  nh->declare_parameter<bool>("pathRangeBySpeed", true);
  nh->declare_parameter<bool>("pathCropByGoal", true);
  ...
  nh->declare_parameter<double>("joyToCheckObstacleDelay", 5.0);
  nh->declare_parameter<double>("goalClearRange", 0.5);
  nh->declare_parameter<double>("goalX", 0.0);
  nh->declare_parameter<double>("goalY", 0.0);

  std::string s;
  nh->get_parameter("pathFolder", s);
  pathFolder =
      ament_index_cpp::get_package_share_directory("local_planner") + "/" + s;
  printf("\nPath folder: %s\n", pathFolder.c_str());
  ...
  auto subOdometry = nh->create_subscription<nav_msgs::msg::Odometry>(
      "/state_estimation", qos, odometryHandler);

  auto subLaserCloud = nh->create_subscription<sensor_msgs::msg::LaserScan>(
      "/scan", qos, laserCloudHandler);

  auto subTerrainCloud = nh->create_subscription<sensor_msgs::msg::PointCloud2>(
      "/terrain_map", qos, terrainCloudHandler);
  ...
  auto subGoal = nh->create_subscription<geometry_msgs::msg::PointStamped>(
      "/way_point", qos, goalHandler);

  auto subSpeed = nh->create_subscription<std_msgs::msg::Float32>("/speed", qos,
                                                                  speedHandler);

  auto subBoundary =
      nh->create_subscription<geometry_msgs::msg::PolygonStamped>(
          "/navigation_boundary", 5, boundaryHandler);

  auto subAddedObstacles =
      nh->create_subscription<sensor_msgs::msg::PointCloud2>(
          "/added_obstacles", qos, addedObstaclesHandler);

  auto subCheckObstacle = nh->create_subscription<std_msgs::msg::Bool>(
      "/check_obstacle", qos, checkObstacleHandler);

  auto pubPath = nh->create_publisher<nav_msgs::msg::Path>("/path", qos);
  nav_msgs::msg::Path path;

#if PLOTPATHSET == 1
  auto pubFreePaths = nh->create_publisher<sensor_msgs::msg::PointCloud2>(
      "/free_paths", rclcpp::QoS(rclcpp::KeepLast(5)));
#endif
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      while (pathScale >= minPathScale && pathRange >= minPathRange) {
        ...
        if (selectedGroupID >= 0) {
          ...
          path.header.stamp = rclcpp::Time(static_cast<int>(odomTime * 1e9));
      ...
      if (!pathFound) {
        ...
        path.header.stamp = rclcpp::Time(static_cast<int64_t>(odomTime * 1e9));
        path.header.frame_id = "vehicle";
        pubPath->publish(path);
      ...
      plannerCloud2.header.stamp = rclcpp::Time(static_cast<int64_t>(odomTime);
